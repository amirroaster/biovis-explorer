...
export class PubchemService {
  ...
  searchByFormula(formula: string): Observable<any> {
  ...
  getBulkCompounds(cids: number[]): Observable<Compound[]> {
    if (!cids || cids.length === 0) {
      return throwError('No CIDs provided');
    }
  ...
  private handleError(error: any) {
    ...
    return throwError('Something went wrong with the PubChem API. Please try again.');
  }
}
